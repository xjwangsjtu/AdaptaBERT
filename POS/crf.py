...
class CRF(nn.Module):
    ...
    def __init__(
        self, nb_labels, bos_tag_id=None, eos_tag_id=None, pad_tag_id=True, batch_first=True, device=None
    ):
        super().__init__()

        self.nb_labels = nb_labels + 2 # bos and eos
        self.BOS_TAG_ID = nb_labels
        self.EOS_TAG_ID = nb_labels + 1
        # padding is on
        self.nb_labels += 1
        self.PAD_TAG_ID = nb_labels + 2
    ...
    def init_weights(self):
        ...
        # no transition allowed from the end of sentence
    ...
    def log_likelihood(self, emissions, tags, mask=None): # attention_mask - [1,1,1,1,0], label_mask - [1,0,1,0,0]
    ...
    def _compute_scores(self, emissions, tags, mask):
        ...
        first_tags = tags[:, 0]
        last_valid_idx = mask.int().sum(1) - 1 # cannot use label_mask for mask
